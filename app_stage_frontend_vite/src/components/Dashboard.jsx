        ...
        <div className="space-y-12 font-['Work_Sans'] animate-in fade-in duration-1000 pb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8">
                {kpiCards.map((kpi, idx) => (
                    <div key={idx} className="bg-white p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-blue-900/5 transition-all group active:scale-95">
                        <div className="flex items-center justify-between mb-4 lg:mb-6">
                            <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-6 ${kpi.bg} ${kpi.color}`}>
                                <kpi.icon size={24} className="lg:hidden" />
                                <kpi.icon size={28} className="hidden lg:block" />
                            </div>
                        </div>
                        <h3 className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">{kpi.title}</h3>
                        <p className="text-3xl lg:text-5xl font-black text-slate-900 tracking-tighter leading-none">{kpi.value}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-4 lg:gap-6">
                <button onClick={() => navigate('/commandes')} className="flex items-center justify-center gap-4 bg-white text-[#075E80] border-2 border-slate-100 px-6 lg:px-10 py-4 lg:py-6 rounded-2xl lg:rounded-[2rem] font-black hover:border-[#075E80] hover:bg-[#075E80] hover:text-white transition-all shadow-sm active:scale-95 uppercase tracking-widest text-[10px] group">
                    <Plus size={20} strokeWidth={3} className="group-hover:rotate-90 transition-transform" /> Nouvelle Commande
                </button>
                <button onClick={() => navigate('/transferts')} className="flex items-center justify-center gap-4 bg-[#075E80] text-white px-6 lg:px-10 py-4 lg:py-6 rounded-2xl lg:rounded-[2rem] font-black hover:bg-slate-900 transition-all shadow-2xl shadow-blue-900/20 active:scale-95 uppercase tracking-widest text-[10px] group border border-white/10">
                    <ArrowLeftRight size={20} strokeWidth={3} className="group-hover:rotate-180 transition-transform duration-500" /> Transferts
                </button>
                <button onClick={() => navigate('/produits')} className="flex items-center justify-center gap-4 bg-white text-slate-600 border-2 border-slate-100 px-6 lg:px-10 py-4 lg:py-6 rounded-2xl lg:rounded-[2rem] font-black hover:border-slate-400 transition-all shadow-sm active:scale-95 uppercase tracking-widest text-[10px] group">
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                {kpiCards.map((kpi, idx) => (
                    <button key={idx} onClick={kpi.action}
                        className="bg-white p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all group active:scale-95 text-left">
                        <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center mb-4 lg:mb-5 transition-transform group-hover:scale-110 ${kpi.bg} ${kpi.color}`}>
                            <kpi.icon size={20} className="lg:hidden" />
                            <kpi.icon size={24} className="hidden lg:block" />
                        </div>
                        <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{kpi.title}</p>
                        <p className="text-3xl lg:text-4xl font-black text-slate-900 tracking-tighter">{kpi.value}</p>
                    </button>
                ))}
            </div>

            {/* Stock Banner */}
            <div className="bg-gradient-to-r from-[#1a2b4b] to-[#075E80] rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-10 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl shadow-blue-900/30">
                <div className="flex flex-col md:flex-row items-center text-center md:text-left gap-4 lg:gap-6">
                    <div className="w-14 h-14 lg:w-16 lg:h-16 bg-white/10 rounded-2xl flex items-center justify-center border border-white/10">
                        <Layers size={28} className="lg:hidden" />
                        <Layers size={32} className="hidden lg:block" />
                    </div>
                    <div>
                ...
                <button onClick={() => navigate('/produits')}
                    className="w-full md:w-auto flex items-center justify-center gap-3 bg-white/10 hover:bg-white/20 border border-white/10 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95">
